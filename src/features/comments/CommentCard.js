...
const CommentCard = ({ comment }) => {
    ...
    const ExpandComment = () => {
        return (
            <span onClick={() => setIsCommentOpen(true)} className='expandComment'>{'(see full comment)'}</span>
        )
    }
    ...
    return (
        <AnimatePresence>
            {!isCommentDeleted && (
                ...
                >
                    <p>{comment.author} <TimeAgo timestamp={comment.date} /></p>

                    {roles && (roles.includes(admin) || comment.author === auth.user) &&
                        <p className='deleteComment' onClick={async () => { await deleteComment() }}>delete</p>
                    }

                    {
                        ...
                        isCommentOpen
                            ...
                            : comment.content.length >= 150 && windowWidth < 700
                                ? <p>{comment.content.substring(0, 150)} <ExpandComment /></p>
                                // Check if the comment content is greater than 250 characters
                                : comment.content.length > 250
                                    ? <p>{comment.content.substring(0, 250)} <ExpandComment /></p>
